    ...
    >
      ...
      {champVersion && (
        <div style={{ marginBottom: 16, padding: '10px 16px', background: '#f0f9ff', borderRadius: 8, borderLeft: '3px solid #0891b2', fontSize: 12 }}>
          ...
          {champDate && <span style={{ color: 'var(--color-muted)', marginLeft: 12 }}>Computed: {fmtDateTime(champDate)}</span>}
          {!hasShap && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--color-muted)' }}>from feature_importance</span>
          </div>
          {isLoading ? <TableSkeleton rows={10} /> : features.length > 0 ? (
            <ResponsiveContainer width="100%" height={Math.max(300, features.length * 22)}>
              <BarChart data={features} layout="vertical" margin={{ left: 40, right: 20 }}>
                ...
                <XAxis type="number" tick={{ fontSize: 11, fill: 'var(--color-muted)' }} tickFormatter={v => v.toFixed(3)} />
                <YAxis dataKey="feature_name" type="category" tick={{ fontSize: 10, fill: 'var(--color-text)' }} width={190} />
                <Tooltip
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 300, color: 'var(--color-muted)' }}>
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--color-muted)' }}>{features.length} features</span>
          </div>
            ...
            <div style={{ maxHeight: 500, overflowY: 'auto' }}>
              <table className="gp-table">
                ...
                <tbody>
                  {features.map((f, i) => (
                    <tr key={i}>
                      <td style={{ color: 'var(--color-muted)', width: 30 }}>{i + 1}</td>
                      ...
                      <td style={{ color: '#7c3aed' }}>
                        {f.shap_mean != null
                          ...
                          : <span style={{ color: 'var(--color-muted)', fontSize: 11 }}>N/A</span>}
                      </td>
                      <td>
                  ...
                  {features.length === 0 && (
                    <tr>
                      <td colSpan={5} style={{ textAlign: 'center', color: 'var(--color-muted)', padding: '40px 0' }}>
